...
@endif
    <table id="example1" class="table table-bordered table-striped">
        ...
        @foreach($socialmedia as $socialmediadata)
            ...
            <tr>
                <td>{{$socialmediadata->id }}</td>
                <td>{{$socialmediadata->name }}</td>
                <td>{{$socialmediadata->link }}</td>
                <td>
            @foreach(json_decode($socialmediadata->images) as $image)
            <img src="{{ $image }}" width="50" height="50" alt="Image">
            @endforeach
        </td>
                
                <td>
                <a href="{{route('SocialMedia.show',$socialmediadata->id)}}" class="btn btn-sm btn-secondary">Show</a>
                <a href="{{route('SocialMedia.edit',$socialmediadata->id)}}" class="btn btn-sm btn-primary">Edit</a>
                <form action="{{ route('SocialMedia.destroy', $socialmediadata->id) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
