...
<div class="content-wrapper">
    ...
    <div class="container">
        <form method="POST" action="{{route('banner.update', $bannerslider->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            @if($errors->any())
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="bannername" value="{{$bannerslider->bannername}}"
                            name="bannername" required>
                    </div>
                </div>
            </div>
        </form>
        @if ($bannerslider->images)
        <div id="current-images" class="row">
            @foreach(json_decode($bannerslider->images) as $index => $image)
            <div class="col-md-3">
                <img src="{{ $image }}" width="200px" height="200px" alt="Current Image">
                <form method="POST"
                    action="{{ route('bannerslider.delete-image', ['id' => $bannerslider->id, 'index' => $index]) }}">
                    @csrf
                    @method('DELETE')
