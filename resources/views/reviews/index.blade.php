    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        @foreach($review as $reviews)
            ...
            <tr>
                ...
                <td>{{$reviews->activity_id }}</td>
                <td>{{$reviews->activity_type }}</td>
                <td>{{$reviews->review }}</td>
                <td>{{$reviews->rating }}</td>
                <td>
                <a href="{{route('review.show',$reviews->id)}}" class="btn btn-sm btn-secondary">Show</a>
                <form action="{{ route('review.destroy', $reviews->id) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
