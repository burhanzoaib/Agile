...
@endif
    <table id="example1" class="table table-bordered table-striped">
        ...
        @foreach($navbar as $navbardata)
            ...
            <tr>
                <td>{{$navbardata->id }}</td>
                <td>{{$navbardata->name }}</td>
                <td>{{$navbardata->link }}</td>
             
                
                <td>
                <a href="{{route('navbar.show',$navbardata->id)}}" class="btn btn-sm btn-secondary">Show</a>
                <a href="{{route('navbar.edit',$navbardata->id)}}" class="btn btn-sm btn-secondary">Edit</a>
                <form action="{{ route('navbar.destroy', $navbardata->id) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
