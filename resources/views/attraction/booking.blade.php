    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        @foreach($booking as $bookings)
            <tr>
                ...
                <td>{{ $bookings->booking_status == 1 ? 'Confirmed' : 'Pending' }}</td>
                <td>{{ $bookings->payment_status == 1 ? 'Confirmed' : 'Pending' }}</td>
                @can('attractionbooking show')
                <td>
                    <a href="{{route('attraction.showbooking',$bookings->id)}}" class="btn btn-sm btn-secondary">Show</a>
                </td>
                @endcan
            </tr>
            @endforeach
